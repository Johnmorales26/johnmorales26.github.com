...
class BottomVisibleOptionsAdapter(private val optionsList: MutableList<ConversionsData>, private val listener: OnBottomOptions)
    : RecyclerView.Adapter<BottomVisibleOptionsAdapter.ViewHolder>() {
        ...
        with(holder){
            binding.apply {
                ...
                cvContainer.setOnClickListener {
                    if (cvContainer.isChecked){
                        cvContainer.isChecked = false
                        option.visibility = false
                    } else {
                        cvContainer.isChecked = true
