...
import com.johndev.neurontraining.AutomaticChartActivity.Companion.paintResultsAutomatic
import com.johndev.neurontraining.BottomOptions.BottomOptionsFragment
import com.johndev.neurontraining.Models.ResultsAuto
...
class DialogPrintAutoFragment(private val onSubmitClickListener: (Float) -> Unit): DialogFragment() {
    ...
    private fun ordenateAnswer() {
        ...
        var i = 0
        while (i < paintResultsAutomatic.size){
            resultList.add(ResultsAuto(paintResultsAutomatic[i].id, paintResultsAutomatic[i].W0,
                paintResultsAutomatic[i].W1, paintResultsAutomatic[i].J))
            i++
        }
    ...
    private fun createJSON(resultList: MutableList<ResultsAuto>) {
        val gson = Gson()
    ...
    private fun createTXT(resultList: MutableList<ResultsAuto>) {
        var exportTXT = ""
        resultList.forEach {
    ...
    private fun createCSV(resultList: MutableList<ResultsAuto>) {
