...
class SearchAdapter(private var searchList: MutableList<SearchData>, val listener: OnPressedSearch):
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder) {
            binding.let { 
                setListener(searchData)
                it.imgLogo.load("${Constans.IMAGES_URL}${searchData.posterPath}")
                it.tvName.text = searchData.name.toString().trim()
                it.tvOriginCountry.text = searchData.id.toString().trim()
            }
        }
    }
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view){
        ...
        fun setListener(searchData: SearchData){
            binding.root.setOnClickListener {
                listener.onSearchPressed(searchData)
            }
        }
