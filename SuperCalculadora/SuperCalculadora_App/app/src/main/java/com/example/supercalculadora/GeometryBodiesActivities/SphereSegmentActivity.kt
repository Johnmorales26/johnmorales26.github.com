...
import com.example.supercalculadora.databinding.ActivitySphereSegmentBinding
import com.example.supercalculadora.functionscalculator.Geometry3D

class SphereSegmentActivity : AppCompatActivity() {
    private lateinit var binding: ActivitySphereSegmentBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivitySphereSegmentBinding.inflate(layoutInflater)
        ...
        binding.btnCalculate.setOnClickListener{
            if (validFields()){
                ...
                with(binding){
                    tvResultVolume.text = functions3D.volumeSphereSegment(radioA, radioB, height)
                }
            }
        }
    }
