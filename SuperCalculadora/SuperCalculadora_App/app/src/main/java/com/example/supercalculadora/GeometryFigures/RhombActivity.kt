...
import com.example.supercalculadora.databinding.ActivityRhombBinding
import com.example.supercalculadora.functionscalculator.Geometry2D

class RhombActivity : AppCompatActivity() {
    private lateinit var binding: ActivityRhombBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityRhombBinding.inflate(layoutInflater)
        ...
        binding.btnCalculate.setOnClickListener{
            if (validFields()){
                ...
                with(binding){
                    tvResultArea.text = triangleFunctions.areaRhomb(sideA, sideB)
                    tvResultPerimeter.text = triangleFunctions.perimeterRhomb(sideA, sideB)
                }
            }
        }
    }
